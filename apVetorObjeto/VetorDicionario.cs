...
class VetorDicionario
{
  ...
  public bool ExisteSemOrdem(string palavraProcurada, out int ondeEsta)  // pesquisa sequencial em vetor desordenado
  {
    ...
    {
      if (dados[ondeEsta].Palavra.Trim() == palavraProcurada) //metodo para buscar a palavra
         achou = true; // achamos chave igual à procurada, então paramos o while (achou vSá true) 
      else
    ...
    while (!achou && !fim) // not (achou or fim) // De Morgan
      if (onde >= qtosDados) // condição i
        ...
        if (dados[onde].Palavra == palavraProcurada)
          ...
          if (dados[onde].Palavra.Length > palavraProcurada.Length)
  ...
  public bool Existe(string palavraProcurada, out int onde)  // onde --> posicao onde achou ou onde deveria estar (inclusão)
  {
    ...
    {
      onde = (inicio + fim) / 2;
      if (palavraProcurada.Trim().ToUpper() == dados[onde].Palavra.Trim())
  ...
  public void OrdenarPorSalario(ListSortDirection qualOrdem)
  {
    if (qualOrdem == ListSortDirection.Ascending)   // ordem crescente
    {
      for (int lento = 0; lento < qtosDados; lento++)
        for (int rapido = lento + 1; rapido < qtosDados; rapido++)
          if (dados[lento].Palavra.CompareTo(dados[rapido].Palavra) > 0) // salários fora de ordem
            Trocar(lento, rapido);
    }
    else
      for (int lento = 0; lento < qtosDados; lento++)
